...
- An addition `+` is defined as addition module 2^32. Basically, this means that we cut off everything that wouldn't fit into a 32-bit word. Note that in the Java implementation at ([2]), regular addition is used. This is because we work with 32-bit integers here, so an addition is implicitly modulo 2^32.
